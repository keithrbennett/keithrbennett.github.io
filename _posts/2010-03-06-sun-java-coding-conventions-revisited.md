...
_The frequency of comments sometimes reflects poor quality of code. When you feel compelled to add a comment, consider rewriting the code to make it clearer.
...
_All source files should begin with a c-style comment that lists the programmer(s), the date, a
...
_Put declarations only at the beginning of blocks.
...
_An empty for statement (one in which all the work is done in the initialization, condition, and update clauses) should have the following form:
...
_A switch statement should have the following form:
...
_One blank line should always be used in the following circumstances:
...
_Variable names should be short yet meaningful.
...
_Use XXX in a comment to flag something that is bogus but works. Use FIXME to flag something
